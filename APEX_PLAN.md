...
6. `.env.local.example` with `ANTHROPIC_API_KEY=`.
...
11. `POST /api/analyze`: Add at the top of the file (outside the handler):
    ...
    ```
    And configure the body size at the bottom:
    ```ts
    export const config = { api: { bodyParser: { sizeLimit: '10mb' } } }
    ```
    Body: `{ front: {base64,mediaType}, side?: {...}, profile }`. Build a
    `messages.create` call: system = `FACE_ANALYSIS_PROMPT`; user message =
    image block(s) + a text block summarizing the profile. Request JSON, run
    result through `safeParseJSON<FaceAnalysis>`. On parse failure, retry ONCE
    with an appended "return ONLY valid JSON" instruction. Return the parsed
